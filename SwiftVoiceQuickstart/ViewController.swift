...
let baseURLString = "https://d8dd-2405-204-9015-861d-91f3-22c6-7b43-d2e0.ngrok.io/"
let accessTokenEndpoint = "accessToken?identity=ravi"
...
class ViewController: UIViewController {
    ...
    */
    ...
    func setupLoader() {
        hud = JGProgressHUD()
        hud.textLabel.text = "Authentication..."
    }
    ...
    func failedToFetchAccessToken() {
        let alertController = UIAlertController(title: "Failed!",
                                                message: "Failed to fetch access token! Please try again",
...
//        guard let token = fetchAccessToken() else {
...
//        accessToken = token
    ...
    func fetchAccessToken() {
        ...
        let param = ["username": "jeremy.mercer@401auto.ca", "password" : "Test@Test123",
                     "grantType": "password", "deviceId" : "fqwtTes_vvqnmsoa0IZI3G:APA91bGzrXP8R2viE5RwJ0TimU4wOmIG5CvWLOwrH7L7FsNB_dTGCa_Jb0j0m2jpNCigaDJBWokxtONzbAg1_1oFo2jh6cB27-PpTnDKRgyivO5Op5s-BvA_r4A49dQLPJ81OmoqtySC",
                     "deviceType" : "WEB"]
        
        hud.show(in: self.view)
        APIManager.loadApiRequest(model:AuthResponse.self, urlPath: "https://api-staging.dabadu.ai/dealers/api/auth/signin", method: .post, params: param, headers: nil) { model, error in
            if let error = error {
                debugPrint(error)
                DispatchQueue.main.async {
                    self.hud.dismiss(animated: true)
                    self.failedToFetchAccessToken()
                }
            } else if let model = model {
                print(model)
                if let token = model.message?.accessToken {
                    APIManager.loadApiRequest(model: AccessTokenResponse.self, urlPath: "https://api-staging.dabadu.ai/messaging/api/phone/generate", method: .get, params: nil, headers: ["Authorization" : "Bearer \(token)"]) { model, error in
                        if let token = model?.message {
                            DispatchQueue.main.async {
                                self.accessToken = token
                                debugPrint("phone/generate \(token)")
                                self.hud.dismiss(animated: true)
                            }
                        } else {
                            DispatchQueue.main.async {
    ...
    func callInviteReceived(callInvite: CallInvite) {
        ...
        let rejectAction = UIAlertAction(title: "Reject", style: .default) { [weak self] (action) in
            if let strongSelf = self {
                ...
                strongSelf.toggleUIState(isEnabled: true, showCallControl: true)
            }
        }
        ...
        if (UIApplication.shared.applicationState != UIApplicationState.active) {
            ...
            content.subtitle = "Call From \(from)"
...
extension ViewController: CallDelegate {
    ...
    func callDidFailToConnect(call: Call, error: Error) {
        ...
        if error.localizedDescription.contains("Access Token expired or expiration date invalid") {
            fetchAccessToken()
        }
    }
...
extension ViewController: CXProviderDelegate {
    ...
    func performVoiceCall(uuid: UUID, client: String?, completionHandler: @escaping (Bool) -> Void) {
        let connectOptions = ConnectOptions(accessToken: accessToken) { builder in
            builder.params = [twimlParamTo: self.outgoingValue.text ?? "", "CallerFirstName": "Vishal"]
            builder.uuid = uuid
        }
